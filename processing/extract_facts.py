...
class FactsExtractor:
    def __init__(self, input_dir: str = "data/aggregated", output_dir: str = "data/facts"):
    ...
    def extract_medium_facts(self, articles: List[Dict]) -> List[Dict[str, Any]]:
        ...
        for i, article in enumerate(articles, 1):
            ...
            try:
                prompt = f"""Extract key technical facts from this Kaspa-related Medium article:
...
Please extract and list specific technical facts, announcements, or insights. Format each fact as:
...
Only include factual, verifiable information. Skip opinions or speculation."""
                ...
                facts_response = self.llm.call_llm(
                    prompt=prompt,
                    system_prompt="You are a technical fact extractor. Focus on verifiable technical facts, developments, and announcements. Be precise and factual."
                )
    ...
    def parse_facts_response(self, response: str, article: Dict) -> List[Dict[str, Any]]:
    ...
    def extract_daily_facts(self, date: str = None) -> Dict[str, Any]:
        ...
        # Medium articles
        medium_facts = self.extract_medium_facts(daily_data["sources"].get("medium_articles", []))
        ...
        fact_stats = {
            ...
            "by_category": {cat: len(facts) for cat, facts in facts_by_category.items()},
            "by_impact": {
    ...
    def run_facts_extraction(self, date: str = None) -> str:
        ...
        print(f"🎯 By impact: High: {impact_stats['high']}, Medium: {impact_stats['medium']}, Low: {impact_stats['low']}")
