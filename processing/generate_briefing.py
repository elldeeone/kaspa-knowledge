...
class BriefingGenerator:
    def __init__(self, input_dir: str = "data/aggregated", output_dir: str = "data/briefings"):
    ...
    def load_daily_data(self, date: str) -> Dict[str, Any]:
        ...
        if not input_path.exists():
            raise FileNotFoundError(f"No aggregated data found for {date} at {input_path}")
    ...
    def generate_medium_briefing(self, articles: List[Dict]) -> Dict[str, Any]:
        ...
        for i, article in enumerate(articles, 1):
            ...
            try:
                prompt = f"""Summarize this Kaspa-related Medium article for a technical briefing:
...
Keep it brief but informative."""
                ...
                summary = self.llm.call_llm(
                    prompt=prompt,
                    system_prompt="You are a technical analyst creating concise summaries for a daily briefing. Focus on actionable insights and technical developments."
                )
        ...
        titles_and_authors = "\n".join([f"- {art['title']} (by {art['author']})" for art in articles])
        
        briefing_prompt = f"""Based on these {len(articles)} Kaspa-related Medium articles, provide a high-level briefing:
...
Keep the briefing concise but comprehensive."""
        ...
        try:
            overall_summary = self.llm.call_llm(
                ...
                system_prompt="You are creating a high-level briefing for Kaspa stakeholders. Focus on strategic insights and technical developments."
            )
        except Exception as e:
        ...
        for article in articles:
            ...
            technical_terms = ["dagknight", "kaspa", "consensus", "pow", "mining", "asic", "optical", "blockchain", "cryptocurrency"]
    ...
    def generate_daily_briefing(self, date: str = None) -> Dict[str, Any]:
        ...
        briefing = {
            ...
            "sources": {
                "medium": self.generate_medium_briefing(daily_data["sources"].get("medium_articles", [])),
                "github": {"summary": "GitHub activity processing not yet implemented."},
                "discord": {"summary": "Discord messages processing not yet implemented."},
                "forum": {"summary": "Forum posts processing not yet implemented."},
                "news": {"summary": "News articles processing not yet implemented."}
            },
            "metadata": {
                "total_sources_processed": len([k for k, v in daily_data["sources"].items() if v]),
    ...
    def run_briefing_generation(self, date: str = None) -> str:
        ...
        print(f"📊 Sources processed: {briefing['metadata']['total_sources_processed']}")
